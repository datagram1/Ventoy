/******************************************************************************
 * partresize.c — Ventoy partition resize utility
 *
 * Copied from Ventoy upstream vtoycli/ with macOS compatibility patches.
 * OWNER: WP2 — Only WP2 may modify this file.
 ******************************************************************************/
...
#ifndef VTOY_SKIP_FAT_IO
#include "../FatIO/include/fat_filelib.h"
#endif
...
static int part_check(const char *disk)
{
    ...
    if (PartStyle == 0)
    {
		...
		printf("DiskSize:%llu NextPartStart:%llu(LBA:%llu) Index:%d\n",
            (unsigned long long)DiskSizeInBytes, (unsigned long long)NextPartStart,
            (unsigned long long)(NextPartStart / 512ULL), Index);
    }
    else
    {
        ...
        for (Count = 0, i = 0; i < 128; i++)
        {
            if (memcmp(&(PartTbl[i].PartGuid), &g_ZeroGuid, sizeof(GUID)))
            {
				printf("GPT Part%d StartLBA:%llu LastLBA:%llu\n", i + 1,
                    (unsigned long long)PartTbl[i].StartLBA,
                    (unsigned long long)PartTbl[i].LastLBA);
                Count++;
            }
        }
	...
	printf("Partition 1 start at: %llu %lluKB, end:%llu, NextPartStart:%llu\n",
		(unsigned long long)Part1Start, (unsigned long long)(Part1Start / 1024),
        (unsigned long long)Part1End, (unsigned long long)NextPartStart);
	...
	if (NextPartStart - Part1End >= VENTOY_EFI_PART_SIZE)
	{
		printf("Free space after partition 1 (%llu) is enough for VTOYEFI part\n",
            (unsigned long long)(NextPartStart - Part1End));
		rc = 1;
	}
...
static int WriteDataToPhyDisk(int fd, UINT64 offset, void *buffer, int len)
{
    ...
    if (newseek != (off_t)offset)
    {
        printf("Failed to lseek %llu %lld %d\n",
            (unsigned long long)offset, (long long)newseek, errno);
        return 0;
    }
...
static int update_part_table(char *disk, UINT64 part2start)
{
    ...
    else
    {
		...
		printf("pGPT->Head.EfiStartLBA=%llu\n", (unsigned long long)pGPT->Head.EfiStartLBA);
		printf("pGPT->Head.EfiBackupLBA=%llu\n", (unsigned long long)pGPT->Head.EfiBackupLBA);
...
#else
        printf("secureboot_proc not available (fat_io_lib not linked)\n");
        return 1;
#endif
    }
